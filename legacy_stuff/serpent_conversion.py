...
def expand_include_cards(lines: List[str], basedir: Path) -> List[str]:
    ...
    ----------
    lines : list of str
        Raw input lines from the primary Serpent input file.
    ...
    -------
    list of str
        Lines with include cards replaced by the referenced file lines.
    """
    index = 0
    while True:
...
def remove_comments(lines: List[str]) -> List[str]:
    ...
    ----------
    lines : list of str
        Raw input lines (with possible comments and whitespace).

    Returns
    -------
    list of str
        Cleaned lines with '%' comments removed and blank lines dropped.
    """
    text = ''.join(lines)
...
def first_word(input: Union[str, List[str]]) -> str:
    ...
    input : str or list of str
        Line text or a pre-split list of tokens.
...
def join_lines(lines: List[str]) -> List[str]:
    ...
    This merges continuation lines until the next recognized card keyword.
    """
    index = 0
    while True:
        # If we've reached end of lines, return
...
def check_unsupported_cards(lines: List[str], keywords: Set[str]):
    """Raise on cards that are explicitly unsupported.
    ...
    lines : list of str
        Input lines to scan.
    keywords : set of str
        Card keywords that should trigger an error.
    """
    for line in lines:
...
def _get_max_numeric_id(lines: List[str], keywords: Set[str], position: int = 1) -> int:
    """Return the maximum numeric ID among specified card types.
    ...
    lines : list of str
        Input lines to scan.
    keywords : set of str
        Card keywords to consider.
    position : int, optional
        Token position where the numeric ID appears.
    """
    max_id = -1
    for line in lines:
        words = line.split()
...
def parse_therm_cards(lines: List[str]) -> Dict[str, str]:
    """Parse thermal scattering cards and map material names to SAB data.

    Returns
    -------
    dict
        Mapping from Serpent thermal material names to OpenMC SAB identifiers.
    """
    therm_materials = {}
    for line in lines:
        words = line.split()
...
def parse_mat_mix_cards(lines: List[str], therm_materials: Dict[str, str]) -> Dict[str, openmc.Material]:
    """Parse material and mixture cards into OpenMC Materials.

    Parameters
    ----------
    lines : list of str
        Preprocessed Serpent input lines.
    therm_materials : dict
        Mapping of thermal material names to SAB data names.

    Returns
    -------
    dict
        Mapping from Serpent material names to OpenMC Material instances.
    """
...
def parse_set_cards(lines: List[str]) -> Dict[str, List[str]]:
    """Parse 'set' cards into a simple options dictionary."""
    options = {}
    for line in lines:
        words = line.split()
...
def parse_trans_cards(lines: List[str]) -> Dict[Tuple[str, str], Dict[str, Any]]:
    """Parse transformation cards into a lookup for later geometry use.

    Returns
    -------
    dict
        Keys are (type, unit) tuples and values carry parsed translation data.
    """
    transformations = {}
    for line in lines:
        words = line.split()
...
def parse_surf_cards(lines: List[str], transformations: Dict[Tuple[str, str], Dict[str, Any]]) -> Dict[str, openmc.Surface]:
    """Parse surface cards and return OpenMC surfaces keyed by name.
    ...
    transformations : dict
        Parsed transformation cards; used for surface translations.
    """
...
def parse_lat_cards(lines: List[str], openmc_universes: Dict[str, universe_fill]):
    """Parse lattice cards and populate the universe/lattice dictionary."""
...
def parse_pin_cards(lines: List[str], materials: Dict[str, openmc.Material], universes: Dict[str, openmc.Universe]):
    """Parse pin cell definitions into new universes."""
...
def parse_cell_cards(
    ...
    """Parse cell cards, assign fills/regions, and track outside cells.

    Returns
    -------
    set of openmc.Cell
        Cells marked as 'outside', used later for boundary inference.
    """
...
def determine_boundary_surfaces(geometry: openmc.Geometry, outside_cells: Set[openmc.Cell]) -> List[openmc.Surface]:
    """Infer boundary candidate surfaces from outside cell halfspaces.

    Parameters
    ----------
    geometry : openmc.Geometry
        Geometry containing the root universe.
    outside_cells : set of openmc.Cell
        Cells flagged as outside by Serpent input.
    """
...
def serpent_to_model(input_file) -> openmc.Model:
    """Convert a Serpent input file into an OpenMC Model."""
...
def main():
    """CLI entry point for converting a Serpent input to model.xml."""
